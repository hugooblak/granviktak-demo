<?php
/**
 * Granvik Tak theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "Granvik Tak Leads" plugin, so leads survive a theme change.
 *
 * @package Granvik Tak
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'granviktak', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'granviktak' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'granviktak' ) ) );

		register_block_pattern_category( 'granviktak-sections', array( 'label' => __( 'Granvik Tak sections', 'granviktak' ) ) );
		register_block_pattern_category( 'granviktak-pages', array( 'label' => __( 'Granvik Tak page layouts', 'granviktak' ) ) );
	}
);
