...
add_filter(
	...
	function ( $exporters ) {
		$exporters['granviktak-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'granviktak-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'granviktak-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'granviktak-leads',
						'group_label' => __( 'Quote requests', 'granviktak-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['granviktak-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'granviktak-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
